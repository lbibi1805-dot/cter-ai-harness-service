...
1. In the project that contains the code that you want to run unit tests on, create the additional source files and write the code necessary for the test cases.
    ...
    The [online GoogleTest documentation](http://google.github.io/googletest/) explains the GTest framework concepts and how to write a test program, starting from individual assertions and working towards a complete program with many test cases. Here, we list a few of the key steps for writing test programs:
...
2. Modify your makefile to dynamically link the GTest library.
    ...
    Although the installer from this latest SDP release includes the GTest library (libgtest.so), you must still [add the library to your project](https://www.qnx.com/developers/docs/8.0/com.qnx.doc.ide.userguide/topic/adding_libraries.html "Our sample application doesn't use libraries but if you want to build an application that does, you need to link those libraries to your application. The way you do this in the IDE depends on the type of makefile you're using."). You must also link your program against the libregex library, by including the -Bdynamic -l regex flags in your linking command. Details on these linker flags are given in the [q++, qcc](https://www.qnx.com/developers/docs/8.0/com.qnx.doc.neutrino.utilities/topic/q/qcc.html) entry in the Utilities Reference.
...
3. Click the Build button (![Icon: Build button](https://www.qnx.com/developers/docs/8.0/com.qnx.doc.ide.userguide/images/icons/build.png)).
