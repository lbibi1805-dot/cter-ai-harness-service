...
export class PollOrchestrator {
  ...
  async pollAllAccounts(): Promise<void> {
    this.pollCount += 1;
    this.logger.interval(this.pollCount);
    ...
    for (const r of stale) this.logger.staleReset(r.fileName, r.updatedAt);
  ...
  private async pollAccount(account: CanvasAccountConfig): Promise<void> {
    ...
    this.logger.fetch(account.index, account.url);

    try {
      await this.pollAccountInner(account, client);
    } catch (err) {
      this.logger.fetchError(account.index, (err as Error).message);
    }
    ...
    if (this.conversationPoller) {
      try {
        ...
        this.logger.info(`[conv] account #${account.index} failed: ${(err as Error).message} — file flow unaffected`);
      }
    }
  }
  ...
  private async pollAccountInner(account: CanvasAccountConfig, client: CanvasClient): Promise<void> {
    ...
    if (!materialsFolder) {
      this.logger.folderSkip(account.index, `"${MATERIALS_FOLDER}" folder not found`);
      return;
    }
    this.logger.folderFound(MATERIALS_FOLDER, materialsFolder.id);
    ...
    if (!qFolder) {
      this.logger.folderSkip(account.index, `"${INPUT_FOLDER}" subfolder not found inside ${MATERIALS_FOLDER}`);
      return;
    }
    this.logger.folderFound(INPUT_FOLDER, qFolder.id);

    let aFolder = subfolders.find((f) => f.name === OUTPUT_FOLDER);
    if (!aFolder) {
      aFolder = await client.createFolder(materialsFolder.id, OUTPUT_FOLDER);
      this.logger.folderCreated(OUTPUT_FOLDER, aFolder.id);
    } else {
      this.logger.folderFound(OUTPUT_FOLDER, aFolder.id);
    }
    ...
    this.logger.fetchDone(account.index, inputFiles.length);
    ...
    for (const file of inputFiles) {
      ...
      if (!parsed) {
        this.logger.skip(file.display_name, 'invalid-name');
        continue;
      }

      if (outputFileNames.has(parsed.doneFileName)) {
        this.logger.skip(file.display_name, 'done-on-canvas');
        continue;
      }
      if (this.state.isProcessed(String(file.id), file.display_name)) {
        this.logger.skip(file.display_name, 'in-state');
        continue;
      }

      this.logger.check(file.display_name);
  ...
  private async processFile(
    ...
    if (parsed.model !== undefined && !isValidModel(parsed.provider, primaryModel)) {
      this.logger.validateFail(file.display_name, parsed.provider, primaryModel, ALLOWED_MODELS[parsed.provider]);
      ...
      await client.uploadFileToFolder(outputFolderId, parsed.doneFileName, resultBuffer, PDF_MIME)
        .catch((err: Error) => this.logger.uploadErrorFailed(parsed.doneFileName, err.message));
      ...
      this.logger.invalidModel(file.display_name, parsed.provider, primaryModel);
      if (account.email) {
    ...
    this.logger.validateOk(file.display_name, parsed.provider, modelChain[0]);
    this.state.setStatus({
      fileId: fileIdStr,
      fileName: file.display_name,
      accountIndex: account.index,
    ...
    for (let mi = 0; mi < modelChain.length; mi++) {
      ...
      for (let attempt = 0; attempt <= this.config.maxRetryCount; attempt++) {
        totalAttempts++;
        try {
          this.logger.download(file.display_name);
          const buffer = await client.downloadFile(file.url);

          this.logger.extract(file.display_name, parsed.extension);
          const content = await extractContent(buffer, parsed.extension);

          this.logger.ai(parsed.provider, currentModel, file.display_name);
          ...
          this.logger.upload(parsed.doneFileName);
          await client.uploadFileToFolder(outputFolderId, parsed.doneFileName, resultBuffer, PDF_MIME);
          ...
          this.logger.ok(file.display_name, parsed.doneFileName);
          if (account.email) {
          ...
          if (isLastAttemptOnLastModel) {
            ...
            this.logger.fallback(parsed.provider, currentModel, modelChain[mi + 1], file.display_name, lastError.message);
          } else {
            this.logger.retry(attempt + 1, this.config.maxRetryCount + 1, file.display_name, lastError.message);
          }
        }
      }
    }
    ...
    this.logger.failed(file.display_name, lastError?.message ?? 'unknown error');
    if (account.email) {
  ...
  private async preparePrompt(content: import('../types').FileContent): Promise<{ systemPrompt: string; fileContent: import('../types').FileContent }> {
    return this.promptService.prepare(content, this.config.systemPrompt, this.config.knowledgeContent, {
      retriever: this.ragRetriever,
      builder: this.citationBuilder,
    });
  }
}
