...
export function loadConfig(): AppConfig {
  ...
  return {
    ...
    defaultModels: {
      ...
      openai: process.env.DEFAULT_MODEL_OPENAI ?? 'gpt-6-astra',
    },
    modelFallback: {
      ...
      openai: (process.env.MODEL_FALLBACK_OPENAI ?? 'gpt-6-astra,gpt-5.3-codex,gpt-5,o3-mini,gpt-4o,gpt-4o-mini').split(',').map(s => s.trim()).filter(Boolean),
    },
